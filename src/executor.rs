...
pub fn spawn_checked<T: 'static, E: 'static, F: Future<Output = Result<T, E>> + 'static>(
    ...
    TaskBuilder::new(future).spawn_checked()
}
...
>(
    ...
    TaskBuilder::new(future).priority(priority).spawn_checked()
}
...
pub fn spawn_with_priority<F: Future + 'static>(
    ...
    TaskBuilder::new(future).priority(priority).spawn()
}

impl<T: 'static, E: 'static, F: Future<Output = Result<T, E>> + 'static> TaskBuilder<F> {
...
impl<F: Future + 'static> TaskBuilder<F> {
    ...
    fn into_parts(self) -> (F, task::TaskMetadata, Priority) {
        let Self {
            future,
            priority,
            spawned_at,
...
pub fn block_on<F: Future>(future: F) -> F::Output {
    ...
    let future = {
        ...
        thread_local! {
            static BLOCK_ON_COUNT: RefCell<u64> = const { RefCell::new(0) };
        }

        let location = std::panic::Location::caller();
        let block_on_id = BLOCK_ON_COUNT.with_borrow_mut(|c| {
            *c += 1;
            *c
        });
